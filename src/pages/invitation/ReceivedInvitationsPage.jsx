...
const ReceivedInvitationsPage = () => {
    ...
    const [filter, setFilter] = useState('pending'); // pending, replied, history

    const fetchInvitations = async () => {
        setLoading(true);
        try {
            const data = await getReceivedInvitations(filter);
            setInvitations(data);
        } catch (err) {
        ...
        <MasterLayout>
            ...
            {loading ? (
                ...
                <div className="text-center py-5 bg-white radius-12 shadow-sm border border-dashed">
                    ...
                    <h5 className="text-muted fw-bold">No {filter} invitations found</h5>
